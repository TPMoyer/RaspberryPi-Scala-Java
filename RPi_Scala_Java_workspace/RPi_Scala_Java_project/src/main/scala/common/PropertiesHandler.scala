...
import common.PropertiesDefaultsAndCustomizer._
import utils.MiscellaneousUtils.printAndLog
...
object PropertiesHandler  {
	...
	def propertiesHandler(fid:String, confIn:Config, numPropertiesFiles:Int): Config={
		...
		try{
			...
			case ce:  ConfigException=>  
				...
				//try{sjc=sjc+"_"+pre0Config.getString("logNameAppender")
				//} catch {
				//  case ce: ConfigException=> 
				//    msg="attempted to set the optional logNameAppender, but  property is not present"
				//    println(msg)
				//}
		...
		*/
		...
		//log.info("212 got config.getInt(\"numPropertiesFilesHandled\")="+config.getInt("numPropertiesFilesHandled")+" vs config.getInt(\"numPropertiesFiles\")="+config.getInt("numPropertiesFiles"))
		...
		if(System.getProperty("os.name").startsWith("Win")){
			...
			var wudel=""
			try {
				wudel=config.getString("winutilsDotExeLocation")
				if(null != System.getProperty("hadoop.home.dir")){
					if(!new File(wudel).exists){
						log.info("succesful infrastructure implacement to pre-empt  \"Failed to locate the winutils binary in the hadoop binary path\"") 
						log.info("did this with System.SetProperty(\"hadoop.home.dir\") with .properties value of   winutilsDotExeLocation="+config.getString("winutilsDotExeLocation"))
						System.setProperty("hadoop.home.dir", wudel)
					}
				}
			} catch {
				case e: com.typesafe.config.ConfigException =>
					val hh=System.getenv("HADOOP_HOME")
					if(0<hh.length){
						if(!new File(hh+"/winutils.exe").exists){
							log.info("see winutils.exe at location of environment variable HADOOP_HOME") 
							val propA = new Properties()
							propA.setProperty("winutilsDotExeLocation",hh+"/winutils.exe")
							config=ConfigFactory.parseProperties(propA).withFallback(config)
						}
					}
